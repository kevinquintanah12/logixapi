...
# Mutación para crear un Chofer (por el administrador asignándole un usuario)
class CreateChofer(graphene.Mutation):
    class Arguments:
        user_id = graphene.Int(required=True)  # Nuevo argumento para pasar el id del usuario.
    ...
    def mutate(self, info, user_id, nombre, apellidos, rfc, licencia, certificaciones, horario_id):
        # Se puede validar que el usuario actual (administrador) tenga permisos
        # para crear un chofer. Aquí asumimos que ya se hizo la validación o que el admin lo tiene permitido.
        ...
        # Verificar que el usuario no tenga ya un chofer asignado, ya que la relación es OneToOne.
        if Chofer.objects.filter(usuario=usuario).exists():
            raise Exception("El usuario ya tiene un chofer asignado.")
        ...
        # Crear el chofer y asignarle el usuario indicado.
        chofer = Chofer(
            nombre=nombre,
            apellidos=apellidos,
            usuario=usuario,  # Se asigna el usuario que se pasó como parámetro.
            rfc=rfc,
            licencia=licencia,
