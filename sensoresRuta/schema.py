...
# (Guarda este token en settings o en una variable de entorno)
...
class SensorRutaType(DjangoObjectType):
    class Meta:
        model  = SensorRuta
        fields = "__all__"  # incluye place_name si ya lo agregaste en el modelo
...
class CrearSensorRuta(graphene.Mutation):
    ...
    def mutate(self, info, ruta_id, latitud, longitud, temperatura, humedad):
        ...
        if resp.status_code == 200:
            ...
            if feats:
                sensor.place_name = feats[0]["place_name"]
                sensor.save()
...
class Query(graphene.ObjectType):
    ...
    # NUEVA consulta: filtrar sensores por número de guía
    sensores_por_guia          = graphene.List(
        SensorRutaType,
    ...
    def resolve_ruta_por_guia(self, info, numero_guia):
        return Ruta.objects.filter(
            entregas__paquete__numero_guia=numero_guia
        ).first()
    ...
    def resolve_sensores_por_guia(self, info, numero_guia):
        return SensorRuta.objects.filter(
            ruta__entregas__paquete__numero_guia=numero_guia
        ).order_by("-timestamp")


# ——————————————————————————————————————————————————————————————
# 6) Root mutations & subscriptions
# ——————————————————————————————————————————————————————————————
